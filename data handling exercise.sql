...
/* these queries 1) add source ids, and their corresponding values
 to result_merge table that did not have matching ids with result merge
 2) retun source values where source id matches result merge id 
...
set val = s.val
from source s
where result_merge_null_fill.val is null and result_merge_null_fill.id = source.id
...
set val = s.val
from source s
where result_merge_override.id = source.id and source.val is not null 
...
select * from source s
where s.val is null and source.id in (select id from result_append)
